...
describe("DepoExchange Test For Arc1155", () => {
  let deployer: SignerWithAddress;
  let erc20Owner: SignerWithAddress;
  let erc1155Owner: SignerWithAddress;
  ...
  before("Deploy contracts and preconfig", async () => {
    ...
    arc1155Token = await deployContract(
      deployer,
      "Arc1155",
    ...
    console.log("===== Allowing TransferManagerERC1155 for Lazy mint =====");

    await arc1155Token.addOperator(transferManagerERC1155.address);
  });
  ...
  describe("matchAskWithTakerBid", async () => {
    ...
    before(async () => {
      ...
      makeOrder = {
        isOrderAsk: true,
        signer: erc1155Owner.address,
        collection: arc1155Token.address,
        price: 10,
        tokenId: 1,
        amount: 3, // can be bigger than 1 for ERC1155
    ...
    it("match should be confirmed", async () => {
      ...
      expect(await arc1155Token.balanceOf(erc1155Owner.address, 1)).to.be.eq(0);
      ...
      expect(await arc1155Token.balanceOf(erc20Owner.address, 1)).to.be.eq(3);
    });

    it("should get the correct uri", async () => {
      expect(await arc1155Token.uri(1)).to.be.eq("https://images.arc.market/1");
    });
  });
  ...
  describe("matchBidWithTakerAsk", async () => {
    ...
    it("match should be confirmed", async () => {
      ...
      expect(await arc1155Token.balanceOf(erc1155Owner.address, 2)).to.be.eq(0);

      await depoExchange
        .connect(erc1155Owner)
        .matchBidWithTakerAsk(takeOrder, signedMakeOrder);

      const afterBalance = await erc20Token.balanceOf(erc20Owner.address);
      expect(beforBalance.sub(takeOrder.price)).to.be.equal(afterBalance);

      expect(await arc1155Token.balanceOf(erc20Owner.address, 2)).to.be.eq(3);
    });

    it("should get the correct uri", async () => {
      expect(await arc1155Token.uri(2)).to.be.eq("https://images.arc.market/2");
    });
  });
});
